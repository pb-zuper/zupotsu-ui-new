...
const ManageBanner: React.FC = () => {
    const headers = ['Banner Name', 'Sub Heading', "Position", "Created by/on", "Publish", 'Actions'];
    const headers2 = ['Bannner Name', "Position"]
    ...
    const columns: any = [
...

        {
            ...
            cellStyle: {
                ...
                fontSize: "14px",
                lineHeight: "21px",
                fontFamily: 'Inter', fontWeight: "500", padding: 0, flexDirection: 'row',
                alignItems: 'center',
                justifyContent: 'flex-start',
            }
        },
        // { field: 'publish', cellStyle: { height: "30px", fontSize: "14px", lineHeight: "21px", fontFamily: 'Inter', fontWeight: "500",padding:0 } },
        {
            field: 'publish',
            render: (_: any, item: any) => (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: "center", padding: 0, border: "0px solid transparent", width: '100%', height: '60px', borderRadius: '8px' }}>
                    <CustomSwitch
                        ...
                        onChange={() => {
                            setischecked({ id: item.id, value: item.publish, type: "publish", status: item.status })
                            setpopup(true)
                        }}
                    />
                </div>),
        ...
        },
        {
            ...
            render: (_: any, item: any) => (
                <ActionButton
                    onEdit={() => { navigation(`/createBanner?id=${item.id}`) }}
                    onDelete={() => {
    ...
    useEffect(() => {
        ...
        const fetchAndTrack = async () => {
            ...
            mixpanelEvents.onLoad(loadTime, 'Manage Banners');
        };
        fetchAndTrack();
        return () => {
            const timeSpent = performance.now() - startTime;
            mixpanelEvents.onUnload('Manage Banners', timeSpent);
        };
    }, [])
                    ...
                    arr.push(
                        {
                            ...
                            "action": "Edit",
                            "status": item.asset ? (item.asset.asset_detail[0].asset_status) : null
                        }
                    )
                })
                setCurrentItems(arr);
    ...
    const publishFun = () => {
        setLoader(true)
        apis.publishBanner(ischecked.id)
            .then((response: any) => {
                onLoad()
                setpopup(false)
            })
    ...
    const deleteFun = () => {
        setLoader(true)
        apis.deleteBanner(ischecked.id)
            .then((response: any) => {
                onLoad()
                setpopup(false)
            })
    ...
    if (!loader) {
        return (
            <Grid item xs={12} md={12} lg={12} sx={{ backgroundColor: 'rgb(250,250,250)', height: '90vh', overflowY: "scroll", overflowX: 'hidden' }}>
                            ...
                            >
                                <div style={{
                                    ...
                                    <Button
                                        ...
                                        onClick={() => { navigation("/createBanner") }}
                                    >
                ...
                >
                    <div style={{ width: deviceType === 'mobile' ? '70%' : '50%', height: '60%', border: "0px solid #FFF", backgroundColor: '#FFF', borderRadius: '24px', position: 'absolute', transition: 'all 0.3s ease-in-out', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                        ...
                        <div style={{ padding: "10px", display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', backgroundColor: "#FFF", border: '0px solid #FFF', width: "100%", borderRadius: '24px' }} >
                            <div style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: "space-between", alignItems: 'center', padding: '10px', paddingLeft: '40px', paddingRight: '20px', backgroundColor: '#FFF', borderTopLeftRadius: '24px', borderTopRightRadius: '24px', }} >
                                <p style={{ fontFamily: "Inter", fontSize: "16px", fontWeight: 600, lineHeight: "19.36px", letterSpacing: "0.02em", margin: 0 }}>Rearrange Banner's</p>
                                <Close
                ...
                <Popup open={popup}
                    setOpen={setpopup}
                    text={"Are you sure you want to " + (ischecked.type == "publish" ? ischecked.value ? "UnPublish ?" : ischecked.status == "private" ? "Publish Private Asset?" : "Publish?" : "") + (ischecked.type == "delete" ? "Delete?" : "")}
                    ...
                    warning={ischecked?.status == "private" ? true : false}
                />
