...
const ZupotsuBuyer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchAndTrack = async () => {
      ...
      mixpanelEvents.onLoad(loadTime, 'Zupotsu Buyers Listing Page');
    };
    fetchAndTrack();
    return () => {
      const timeSpent = performance.now() - startTime;
      mixpanelEvents.onUnload('Zupotsu Buyers Listing Page', timeSpent);
    };
  }, [callApi])
    ...
    apis.getUsers()
      ...
      .catch((error) => {
        ...
        mixpanelEvents.errorHandling({
          name: 'Zupotsu Buyers Listing Page',
  ...
  const onUserActivateDisable = () => {
    ...
    const body = {
      ...
      ...(editBuyer?.is_active ? { "disable_reason": disableReason } : {})
    };
  ...
  const deleteUser = () => {
    if (!reason) {
      ...
      apis.userDelete(editBuyer?.id, body)
        .then((response: any) => {
          if (response?.data?.status == "success") {
            setCallApi(!callApi)
            setSnackbar({
  ...
  const onBuyerUpdation = () => {

    setLoader(true);

    if (popuptype === "edit") {
      if (!editBuyer?.name || !editBuyer?.mobile || editBuyer?.mobile?.length < 10) {
        setLoader(false);
        setSnackbar({
          open: true,
          severity: 'error',
  ...
  const onRoleChange = () => {
    setLoader(true);
    const body = {
      "id": editBuyer?.userroles[0]?.id || "",
      "user_id": editBuyer?.id,
            ...
            <div style={{ width: '100%', display: 'flex', flexDirection: deviceType == "mobile" ? "column" : 'row', justifyContent: deviceType == "mobile" ? "flex-start" : 'space-between', alignItems: deviceType == "mobile" ? "flex-start" : 'center', padding: '10px', marginTop: '10px', marginBottom: '10px', backgroundColor: '#FFF' }}>
              ...
              >
                <div style={{
                  ...
                  >
                    <span style={{ marginRight: '8px' }}>+</span>Invite Buyer
                  </Button>
          ...
          >
            ...
            <Box sx={style}>
              ...
              {(popuptype !== "sendpasswordlink") && (<div style={{
                ...
                >
                  {popuptype == "user" ? "Invite Buyer" : popuptype == "disableuser" ? (editBuyer?.is_active ? "Disable User" : "Activate User") : popuptype == "changerole" ? "Change Role" : popuptype == "edit" ? "Edit" : "Remove User"}
                </Typography>
                <Close
              ...
              {(popuptype == "disableuser") && (
                <Typography sx={{
                  ...
                  Are you sure you
                  want to permanent {editBuyer?.is_active ? "disable" : "enable"} <span style={{ color: "rgba(226, 11, 24, 1)" }}>{editBuyer?.name}?</span>
                </Typography>)}
              {(editBuyer?.is_active && popuptype == "disableuser") && (<ZupotsuTextfield
                  ...
                  <Box sx={{ width: '100%', display: 'flex', flexDirection: "row", justifyContent: 'space-evenly', alignItems: 'center', gap: '10px', marginTop: '10px' }}>
                    <div style={{ display: 'flex', width: '50%', flexDirection: "row", justifyContent: 'space-evenly', alignItems: 'center' }}>
                      <ZupotsuTextfield
                        ...
                        name={"email"}
                        multiline={false}
                        errorMessage={(editBuyer["email"] && !validateEmail(editBuyer["email"])) ? "Email is not valid" : ""}
                        previewMode={popuptype == "edit" ? true : edit}
                        handleChange={(e: any) => { handleEdit(e) }}
                      // maxLength={10}
                      />
              ...
              >
                ...
                <ZupotsuButton
                  ...
                  disabled={(popuptype == "changerole" && (!reason || !newUser)) ? true : (popuptype == "disableuser" && !disableReason && editBuyer?.is_active == true) ? true : (popuptype == "user" && (!userEmail?.trim() || !validateEmail(userEmail?.trim()) || !userName)) ? true : false}
